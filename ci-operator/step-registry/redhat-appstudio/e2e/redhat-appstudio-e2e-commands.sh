...
export DEFAULT_QUAY_ORG DEFAULT_QUAY_ORG_TOKEN GITHUB_USER GITHUB_TOKEN QUAY_TOKEN QUAY_OAUTH_USER QUAY_OAUTH_TOKEN QUAY_OAUTH_TOKEN_RELEASE_SOURCE QUAY_OAUTH_TOKEN_RELEASE_DESTINATION OPENSHIFT_API OPENSHIFT_USERNAME OPENSHIFT_PASSWORD \
    ...
    QE_SPRAYPROXY_HOST QE_SPRAYPROXY_TOKEN PAC_GITHUB_APP_ID PAC_GITHUB_APP_PRIVATE_KEY PAC_GITHUB_APP_WEBHOOK_SECRET
...
PAC_GITHUB_APP_ID=$(cat /usr/local/ci-secrets/redhat-appstudio-qe/pac-github-app-id)
PAC_GITHUB_APP_PRIVATE_KEY=$(cat /usr/local/ci-secrets/redhat-appstudio-qe/pac-github-app-private-key)
